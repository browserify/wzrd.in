...
module.exports = function createCache(options, generate) {
  ...
  return function (body, cb) {
    ...
    log.info('body `' + JSON.stringify(body) + '` has hash `' + hash + '`.');

    log.info('checking leveldb for bundle `' + hash + '`...');

    db.get(hash, function (err, b) {
      if (err && err.name === 'NotFoundError') {

        log.info('did not find bundle `' + hash + '`.');
      ...
      if (!err) {
        log.info('successfully found bundle `' + hash + '`.');
      }
