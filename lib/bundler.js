...
class Bundler {
  ...
  _getAlias(input) {
    ...
    return this._caches.aliases.check(input, () => {
      return this._registry.resolve(input.module_scope, input.module_name, input.module_semver).then((v) => {
        assert(v, 'lolqut');
        return v;
      });
    });
  ...
  _build(input) {
    return Promise.try(() => {
      ...
      this._recordBuildStatus(input, build).catch((err) => { /* TODO: Log error */ });
      return build;
    });
  }
