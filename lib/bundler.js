...
class Bundler {
  ...
  _getAlias(module_name, module_semver) {
    // Special case for core modules
    if (core.test(input.module_name)) {
      // TODO: Note that version is **wrong**
      return Promise.resolve(process.version);
    }

    return this._caches.aliases.check({ module: module, semver: semver }, () => {
  ...
  purge(input) {
    ...
    return Promise.try(() => {
      return this._getAlias(module, semver);
    }).then((node_version) => {
