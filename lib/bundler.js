...
class Bundler {
  constructor(opts) {
    ...
    this._cache = cache.createCache(this._db);
  ...
  _build(input) {
    return Promise.try(() => {
      ...
      return this._builder.build(input).then((output) => {
        ...
        this._cache.statuses.put(input, { ok: true }).then(() => {
          return {
            package: output.pkg,
  ...
  purge(input) {
    ...
    function purgeBundles(version) {
      ...
      return this._cache.aliases.del({ module: module, semver: semver })
        .then(() => {
          delete input.purge;

          return this._cache.bundles.del(pkg, done);
        })
      ;
    }
  }
